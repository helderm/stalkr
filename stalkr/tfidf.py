...
def tf_idf(query):
	...
	for token, count in query.iteritems():
		...
		users = graph.cypher.execute("MATCH (u:User)-[d:DISCUSSES]->(w:Word) WHERE w.name = \"{0}\" RETURN u.screen_name, u.term_count, d.count".format(token))
		# Document frequency
		df = len(users)
...
query = "donald trump politics vote"
